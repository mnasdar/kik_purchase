 ...
 * Mengelola form create purchase request
 *
 * @module modules/purchase-request/purchase-request-create
 */

import { initCreateForm, setupNumberInput } from "../../core/form-handler.js";
...
/**
 * Inisialisasi form create purchase request
 */
function initPRCreate() {
    if (!$("#form-create-pr").length) return;

    initCreateForm({
        formSelector: "#form-create-pr",
        confirmMessage: "Data purchase request akan disimpan ke sistem.",
        successMessage: "Purchase request berhasil ditambahkan.",
        onSuccess: (response) => {
            window.location.href = response.redirect || route("purchase-request.index", { prefix });
        },
        onError: (xhr) => {
            console.error("Error creating PR:", xhr);
        },
        beforeSubmit: () => {
            $("#form-create-pr p[id^='error-']").text("");

            const formData = new FormData(document.getElementById("form-create-pr"));
...
function validatePRForm(formData) {
    ...
    const prNumber = formData.get("pr_number");
    if (!prNumber || prNumber.trim() === "") {
        errors["pr_number"] = "PR Number harus diisi.";
    }
...
export { initPRCreate, validatePRForm, setupPRInputs };
