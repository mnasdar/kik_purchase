...
function initGridTable({
    ...
    new Grid({
        columns: [
            {
                id: "Checkbox",
                name: html(
                    `<div class="form-check"><input type="checkbox" class="form-checkbox rounded text-primary header-checkbox" id="${headerCheckId}"></div>`
                ),
                width: "50px",
                sort: false,
        ...
        sort: true,
        data: () =>
            new Promise((resolve) => setTimeout(() => resolve(gridData), 200)),
    }).render(container);
