/**
 * Modul Invoice - Bulk Edit
 * Menangani bulk edit invoice
 */
import $ from "jquery";
...
function setupFormSubmit() {
    ...
    $form.on("submit", function (e) {
        ...
        const invoiceNumber = $('#bulk_invoice_number').val() || null;
        const receivedDate = $('#bulk_invoice_received_at').val();
        const slaTarget = $('#bulk_sla_target').val();

        // Validation
        if (!receivedDate || !receivedDate.trim()) {
            showToast("Tanggal diterima harus diisi", "warning", 2000);
            return;
        }

        if (!slaTarget || parseInt(slaTarget) < 1) {
            showToast("Target SLA harus minimal 1 hari", "warning", 2000);
            return;
        }
...
function submitUpdate(data, $form) {
    ...
    $.ajax({
        url: route("dari-vendor.bulk-update"),
        method: "POST",
        ...
        success: function (response) {
            ...
            // Redirect
            setTimeout(() => {
                window.location.href = response.redirect || route("dari-vendor.index");
            }, 1500);
        },
        error: function (xhr) {
