@forelse($data as $item)
    @if ($item->trackings->count() > 1)
        @foreach ($item->trackings as $subitem)
            <tr id="row-{{ $item->id }}-{{ $subitem->id }}"
                ...
                <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-800 dark:text-gray-200">
                    {{ $subitem->purchase_request->pr_number }}
                </td>
    ...
    @else
        <tr id="row-{{ $item->id }}"
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-800 dark:text-gray-200">
                {{ $item->trackings->first()->purchase_request->pr_number }}
            </td>
            <td class="px-6 py-4 text-sm text-gray-800 dark:text-gray-200 gap-1">
                {{ $item->po_number }}
                @if ($item->is_new)
